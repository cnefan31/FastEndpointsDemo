...
public interface IEventSubscriberStorageProvider<TStorageRecord> where TStorageRecord : IEventStorageRecord
{
    ...
    Task StoreEventAsync(TStorageRecord r, CancellationToken ct);
    ...
    Task<IEnumerable<TStorageRecord>> GetNextBatchAsync(PendingRecordSearchParams<TStorageRecord> parameters);
    ...
    /// <param name="ct">cancellation token</param>
    Task MarkEventAsCompleteAsync(TStorageRecord r, CancellationToken ct);
    ...
    Task PurgeStaleRecordsAsync(StaleRecordSearchParams<TStorageRecord> parameters);
}